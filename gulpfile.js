...
gulp.task('motionJS', function () {
    return gulp.src(paths.buildMotion)
        .pipe(newer(paths.motionDest))
        .pipe(shell('python <%= file.path %> > ' + paths.motionDest));
});

gulp.task('js', ['lint', 'motionJS'], buildJsTask());

gulp.task('jsWatch', ['lint', 'motionJS'], buildJsTask({watch:true}));
...
gulp.task('copyBotSample', function () {
    return gulp.src('js/vbot/sample_script.js')
        .pipe(gulp.dest(assetPath + 'js/'));
});
...
gulp.task('watch', ['build'], function () {
    ...
    makeWatch('js/vbot/sample_script.js', 'copyBotSample');
});
